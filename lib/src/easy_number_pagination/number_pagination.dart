...
class NumberPagination extends StatelessWidget {
          ...
          (_, __) => Row(
            children: [
              ...
              ControlButton(
                ...
                (c) => _changePage(
                  c,
                  isForward ? totalPages : pageService.currentPage - 1,
                ),
                controlButtonSize,
                controlButtonColor,
                enableInteraction,
          ...
          return Row(
            ...
            children: [
              for (var i = rangeStart; i < rangeEnd; i++) ...[
                ...
                if (i != rangeEnd - 1)
                  SizedBox(width: betweenNumberButtonSpacing),
              ],
            ],
          );
        },
  ...
  int _calculateRangeStart(int currentPage) {
    // 计算当前页居中时的起始页码
    int start = currentPage - (visiblePagesCount ~/ 2);
    ...
    // 确保结束页不超过总页数
    int end = start + visiblePagesCount - 1;
    if (end > totalPages) {
      end = totalPages;
      start = end - visiblePagesCount + 1;
      if (start < 1) {
        start = 1;
      }
    }

    return start - 1; // 减1是因为后续会加1显示
  }
  ...
  /// Calculates the end of the range for page numbers to display.
  int _calculateRangeEnd(int rangeStart) {
    final int end = rangeStart + visiblePagesCount;
