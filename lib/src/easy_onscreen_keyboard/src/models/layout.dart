...
class KeyboardMode {
  ...
  final EasyOnscreenKeyboardThemeData Function(BuildContext context)? theme;
}

/// Represents a single row in a keyboard layout.
///
/// Each row contains a list of [EasyOnscreenKeyboardKey]s
...
class KeyboardRow {
  ...
  const KeyboardRow({required this.keys, this.leading, this.trailing});

  /// The list of keys displayed in this row.
  final List<EasyOnscreenKeyboardKey> keys;
...
class _CustomKeyboardLayout extends KeyboardLayout {
  const _CustomKeyboardLayout({required this.aspectRatio, required this.modes});
