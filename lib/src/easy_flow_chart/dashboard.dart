...
import 'easy_flow_chart_library.dart';

// 组内布局数据
class GroupLayoutData {
  final String id;
  final List<List<FlowElement>> columnsLayoutData; // 所有列及其子节点
  final List<List<FlowElement>> rowsLayoutData; // 所有行及其子节点
  GroupLayoutData({
    required this.id,
...
typedef ConnectionListener =
    void Function(FlowElement srcElement, FlowElement destElement);
...
class Dashboard extends ChangeNotifier {
  ...
  Dashboard({
    ...
    this.defaultArrowStyle = ArrowStyle.rectangular,
       ...
       selectDeviceListForDashboard = [],
       instrumentsForDashboard = [],
  ...
  List<Map<String, dynamic>> selectDeviceListForDashboard;

  List<Map<String, dynamic>> instrumentsForDashboard;
  ...
  void updateAllGroupsLayoutData() {
    Future.microtask(() {
      elements.where((element) => element.taskType == TaskType.group).map((
        groupElement,
      ) {
  ...
  void setFullView() {
    ...
    // 屏幕中心点
    final center =
        Offset(dashboardSize.width / 2, dashboardSize.height / 2) +
        _dashboardPosition;
    gridBackgroundParams.offset = center;
    // 区域偏移
    final currentDeviation = boundingBoxSize.center - center;
    double newZoomFactor =
        calculateScale(boundingBoxSize.size, dashboardSize) +
  ...
  void addElementByPlus(FlowElement sourceElement, FlowElement orderElement) {
    ...
    if (nextElementsConnectionParams.isNotEmpty) {
      ...
      // 调整所有后续节点的垂直位置
      final orderElementBottomHandlerPos = orderElement.getHandlerPosition(
        Alignment.bottomCenter,
      );
      final sourceEleBottomHandlerPos = sourceElement.getHandlerPosition(
        Alignment.bottomCenter,
      );
      ...
      for (int i = 0; i < elements.length; i++) {
        ...
        int elementGroupIndex =
            elementGroupId == ''
                ? -1
                : elements.indexOf(
                  elements.firstWhere((el) => el.id == elementGroupId),
                );
  ...
  void addElementByGroupColumnPlus(
    ...
    final sourceElementColumnIndex = groupElement.colsElementIds.indexWhere(
      (elementIds) => elementIds.contains(sourceElement.id),
    );
    final sourceElementRowIndex = groupElement.rowsElementIds.indexWhere(
      (elementIds) => elementIds.contains(sourceElement.id),
    );
    final columnNum = groupElement.colsElementIds.length;
    for (int i = 0; i < columnNum; i++) {
      if (i == sourceElementColumnIndex) {
        groupElement.colsElementIds[i].insert(
          sourceElementRowIndex + 1,
          orderElement.id,
        );
      } else {
    ...
    groupElement.colsElementIds = cleanLastEmptyItems(
    ...
    if (nextElementsConnectionParams.isNotEmpty) {
      ...
      for (int i = 0; i < bottomOfSourceElementIds.length; i++) {
        ...
        if ((destElement.position.dy > sourceDy.toDouble())) {
          destElement.changePosition(
            Offset(
              destElement.position.dx,
              destElement.position.dy + addHeight,
            ),
          );
        }
      }
      ...
      final newGroupElementSize = Size(
  ...
  void addElementByGroupRightPlus(
    ...
    /// 新增节点(节点先进画布，获取真实宽高)
    addElement(orderElement);
    groupElement.rowsElementIds = List.from(
      groupElement.rowsElementIds,
    ); // 第一行的最后一个元素为orderElement.id，其他行的最后一个元素是""
    ...
    groupElement.rowsElementIds = cleanLastEmptyItems(
      groupElement.rowsElementIds,
    );
  ...
  void updateLayOutAfterDelElementInGroup(
    ...
    final delEleBottomHandlerPos = deletedElement.getHandlerPosition(
      Alignment.bottomCenter,
    );
  ...
  /// 在组内通过 节点[id] 移除节点
  void removeElementInGroupByElementId(
    String removedElementId, {
    bool notify = true,
  }) {
    ...
    final currentGroupElementSpacing = groupElementSpacing * zoomFactor;
    FlowElement? removedEleNextEle =
        removedElement.next.isNotEmpty
    ...
    final lastGroupColumnLayoutData = getGroupColumnLayoutData(
      removedElement.parentId,
    );
    final removedElementColumnIndex = groupElement.colsElementIds.indexWhere(
      (elementIds) => elementIds.contains(removedElementId),
    );
    final removedElementRowIndex = groupElement.rowsElementIds.indexWhere(
      (elementIds) => elementIds.contains(removedElementId),
    );
    ...
    if (removedElementColumnIndex != -1 && removedElementRowIndex != -1) {
      if (removedEleNextEle != null) {
        groupElement.colsElementIds[removedElementColumnIndex].removeAt(
          removedElementRowIndex,
        );
        groupElement.colsElementIds[removedElementColumnIndex].add("");
      } else {
        groupElement
                .colsElementIds[removedElementColumnIndex][removedElementRowIndex] =
            "";
      }
      groupElement.colsElementIds = cleanLastEmptyItems(
        groupElement.colsElementIds,
    ...
    if (sourceElement == null) {
      ...
      if (removedEleNextEle != null) {
        //删除的节点下面有其他节点
        final removedEleBottomHandlerPos = removedElement.getHandlerPosition(
          Alignment.bottomCenter,
        );
        final removedNextEleBottomHandlerPos = removedEleNextEle
            .getHandlerPosition(Alignment.bottomCenter);
        decreaseHeight =
      ...
      final sourceElementBottomHandlerPos = sourceElement.getHandlerPosition(
        Alignment.bottomCenter,
      );
      final removedEleBottomHandlerPos = removedElement.getHandlerPosition(
        Alignment.bottomCenter,
    ...
    for (List<FlowElement> columnElements in lastGroupColumnLayoutData) {
      if (columnElements.first.position.dx == removedElement.position.dx) {
        bottomOfRemovedElements =
            columnElements.where((ele) {
    ...
    final newGroupElementSize = Size(
      groupElement.size.width - decreaseWidth,
  ...
  Map<String, dynamic> toMap() {
    return <String, dynamic>{
      ...
      'selectDeviceListForDashboard': selectDeviceListForDashboard,
      'instrumentsForDashboard': instrumentsForDashboard,
    };
  }
  ...
  /// 从elements中收集stepParams 并 格式化后的json数据
  /// 获取格式化后的stepParams json数据
  get toPrettyStepParamsJsonString {
    final stepParamsList =
        elements.map((element) => element.stepParams).toList();
    return const JsonEncoder.withIndent('  ').convert(stepParamsList);
  }
  ...
  void saveProtocolData(String completeFilePath, dynamic protocolJson) {
    final spaces = ' ' * 2;
    final encoder = JsonEncoder.withIndent(spaces);
    final stringData = encoder.convert(protocolJson);
    File(completeFilePath).writeAsStringSync(stringData, flush: true);
  }
  ...
  void loadDashboardDataByJson(String jsonString) async {
  ...
  void loadDashboardData(Map<String, dynamic> source) {
    ...
    minimumZoomFactor = (source['minimumZoomFactor'] ?? 0.25).toDouble();
    maximumZoomFactor = (source['maximumZoomFactor'] ?? 1.8).toDouble();
    dashboardSize = Size(
      source['dashboardSizeWidth'].toDouble() as double,
      source['dashboardSizeHeight'].toDouble() as double,
    );
  ...
  void setSelectDeviceListForDashboard(
    List<Map<String, dynamic>> deviceListForDashboard,
  ) {
    selectDeviceListForDashboard = deviceListForDashboard;
    print("selectDeviceListForDashboard====>$deviceListForDashboard");
  }

  void setInstrumentsForDashboard(List<Map<String, dynamic>> instruments) {
    instrumentsForDashboard = instruments;
  }
