...
import 'easy_multi_selection.dart';
import 'easy_single_selction.dart';
...
class EasyDropDownTextField extends StatefulWidget {
  const EasyDropDownTextField({
    super.key,
    this.controller,
    this.initialValue,
  ...
  const EasyDropDownTextField.multiSelection({
    super.key,
    this.controller,
  ...
  ///offset width for dropdown
  final double offsetWidth;

  /// 下拉列表最大固定高度，如果为null则自适应
  final double? dropdownListMaxHeight;

  @override
  _EasyDropDownTextFieldState createState() => _EasyDropDownTextFieldState();
}

class _EasyDropDownTextFieldState extends State<EasyDropDownTextField>
  ...
  updateFunction({EasyDropDownTextField? oldWidget}) {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      final maxDropdownHeight = screenHeight * 0.6; // 限制为窗口高度的60%

      double calculatedHeight =
          widget.dropdownListMaxHeight == null
              ? (!widget.isMultiSelection
                      ...
                      : _listTileHeight * _maxListItem.toDouble()) +
                  10
              : widget.dropdownListMaxHeight!; // 如果设置了最大高度，则直接使用该值

      // 确保计算出的高度不超过窗口最大高度限制
      _height = calculatedHeight.clamp(0.0, maxDropdownHeight);
    });
  }

  @override
  void didUpdateWidget(covariant EasyDropDownTextField oldWidget) {
              ...
              validator:
                  (value) =>
                      widget.validator != null
                          ...
                          : null,
              decoration:
                  widget.textFieldDecoration != null
  ...
  Future<void> _showOverlay() async {
    ...
    // 限制下拉列表高度不超过窗口最大高度的80%
    dropdownListHeight = dropdownListHeight.clamp(0, maxHeight * 0.8);
    ...
    // 限制下拉列表宽度不超过窗口最大宽度的90%
    final overlayWidth =
        (size.width + widget.offsetWidth).clamp(0.0, maxWidth * 0.9).toDouble();
    final overlayWidth2 = size.width.clamp(0.0, maxWidth * 0.9).toDouble();
            ...
            (context) => Positioned(
              ...
              child: CompositedTransformFollower(
                ...
                child: AnimatedBuilder(
                  ...
                  builder:
                      (context, child) =>
                          buildOverlay(context, child, overlayKey),
                ),
              ),
            ),
      );
  ...
  Widget buildOverlay(context, child, GlobalKey overlayKey) {
